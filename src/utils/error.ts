...
function isHubSpotApiError(error: any): boolean {
  ...
  const hasHubspotHeaders = error.headers && (
  ...
  const hasHubspotFields = error.body && (
    error.body.status === 'error' &&
    typeof error.body.correlationId === 'string' &&
    typeof error.body.groupsErrorCode === 'string'
  );

  return hasHubspotHeaders || hasHubspotFields ||  Boolean(error?.message?.includes('hubapi') || error?.logMessage.message?.body.includes('hubspot-correlation-id'));
  ;
}
